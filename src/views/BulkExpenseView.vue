  ...
  <AppLayout>
    <div class="p-4 lg:p-8 space-y-6 mb-20 lg:mb-0">
      ...
      <div class="flex justify-between gap-4">
        <div>
          <h1 class="text-2xl lg:text-3xl font-bold text-gray-900 dark:text-neon">Bulk Expense via OCR</h1>
      ...
      <div v-if="currentStep === 1" class="card bg-base-100 shadow p-6">
        <div class="card-body">
          ...
          <p class="text-sm text-gray-500 dark:text-gray-400">Upload a clear image of a receipt or a list of expenses.</p>
            ...
            >
              <div class="text-center">
                ...
                <div class="mt-4 flex text-sm leading-6 text-gray-600 dark:text-gray-400">
                  <label for="file-upload" class="relative cursor-pointer rounded-md font-semibold text-primary-600 dark:text-primary-400 hover:text-primary-500">
                    <span>Upload a file</span>
                    <input id="file-upload" name="file-upload" type="file" class="sr-only" @change="handleImageUpload" accept="image/*">
                  </label>
                  <p class="pl-1">or drag and drop</p>
                </div>
                <p class="text-xs leading-5 text-gray-500 dark:text-gray-500">PNG, JPG, GIF up to 10MB</p>
              </div>
            </div>
          </div>
            ...
            <div class="flex items-center justify-between p-2 border rounded-md bg-gray-50 dark:bg-gray-800/50">
              <div class="flex items-center gap-3">
                ...
                <div class="text-sm font-medium">
                  ...
                  <p class="text-xs text-gray-500">{{ (selectedImage.size / 1024).toFixed(2) }} KB</p>
                </div>
              </div>
          ...
          <div class="card-actions justify-end mt-4 flex items-center">
            <button @click="processImage" class="btn btn-primary flex items-center gap-2" :disabled="!selectedImage || ocrLoading">
      ...
      <div v-if="currentStep === 2" class="card bg-base-100 shadow p-6">
        <div class="card-body">
          ...
          <p class="text-sm text-gray-500 dark:text-gray-400">Correct any errors in the extracted text. This text will be sent to the server for processing.</p>
          
          <div class="mt-4 grid grid-cols-1 lg:grid-cols-2 gap-6">
            <!-- Image Preview Column -->
            <div class="space-y-2">
              <h3 class="text-sm font-medium text-gray-700 dark:text-gray-300">Image Preview</h3>
              <div class="p-2 border rounded-md bg-gray-50 dark:bg-gray-800/50 h-[50vh] flex items-center justify-center">
                <img 
            ...
            <!-- OCR Text Column -->
            <div class="space-y-2">
              <h3 class="text-sm font-medium text-gray-700 dark:text-gray-300">Extracted Text (Editable)</h3>
          ...
          <div class="card-actions justify-end mt-6 flex items-center gap-2">
            ...
            <button @click="previewTransactions" class="btn btn-primary" :disabled="!ocrResultText.trim() || previewLoading">
              <span v-if="previewLoading" class="loading loading-spinner"></span>
      ...
      <div v-if="currentStep === 3" class="card bg-base-100 shadow p-6">
        <div class="card-body">
          ...
          <div class="card-actions justify-end mt-4 flex items-center gap-2">
            ...
            <button @click="saveTransactions" class="btn btn-primary" :disabled="savingLoading">
              <span v-if="savingLoading" class="loading loading-spinner"></span>
...
const isAllSelected = computed(() => previewedTransactions.value.length > 0 && previewedTransactions.value.every(t => t.selected));
...
const processImage = async () => {
  ...
  try {
    const { data: { text } } = await worker.recognize(selectedImage.value);
...
const previewTransactions = async () => {
  ...
  try {
    const response = await transactionsStore.mapExpenseTransaction(ocrResultText.value)

    if (response.data) {
      const defaultWalletId = wallets.value.length > 0 ? wallets.value[0].id : '';
      const defaultCategoryId = expenseCategories.value.length > 0 ? expenseCategories.value[0].id : '';
      const today = format(new Date(), 'yyyy-MM-dd');
      previewedTransactions.value = <PreviewTransaction[]>response.data.map(item => ({
        date: today,
        note: item.note,
...
const saveTransactions = async () => {
  ...
  const transactionsToSave = previewedTransactions.value.filter(t => t.selected);
  
  const promises = transactionsToSave.map(t => transactionsStore.createTransaction({
    type: 'expense',
    amount: t.amount,
  ...
  try {
    ...
    alert('An error occurred while saving transactions. Please check the console for details.');
  } finally {
